...
function t1053_unifyHeights(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var rows=rec.querySelectorAll('.t1053 .t1053__row');for(var i=0;i<rows.length;i++){var container=rows[i];var maxHeight=0;var cols=container.querySelectorAll('.t1053__col');for(var j=0;j<cols.length;j++){cols[j].style.height=''}
for(var j=0;j<cols.length;j++){var col=cols[j];var colStyle=getComputedStyle(col,null);var colPaddingTop=parseInt(colStyle.paddingTop)||0;var colPaddingBottom=parseInt(colStyle.paddingBottom)||0;var colHeight=col.clientHeight-(colPaddingTop+colPaddingBottom);var colWrap=col.querySelector('.t1053__col-wrapper');var colWrapStyle=getComputedStyle(colWrap,null);var colWrapPaddingTop=parseInt(colWrapStyle.paddingTop)||0;var colWrapPaddingBottom=parseInt(colWrapStyle.paddingBottom)||0;var colWrapHeight=colWrap.clientHeight-(colWrapPaddingTop+colWrapPaddingBottom);if(colHeight<colWrapHeight){col.style.height=colWrapHeight+'px';maxHeight=colWrapHeight}
